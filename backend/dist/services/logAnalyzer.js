...
function analyzeLogs(rawLogs, serviceName, incidentTime) {
    ...
    const parsed = lines.map((raw, index) => {
        ...
        if (level === "ERROR" || level === "FATAL")
            ...
            score += 0.3;
        if (raw.toLowerCase().includes(serviceName.toLowerCase()))
            score += 0.2;
        if (timestamp) {
    ...
    const topLines = [...parsed]
        ...
        .slice(0, 60);
    const errorLines = parsed.filter((l) => l.level === "ERROR" || l.level === "FATAL");
    ...
    return {
        ...
        warnCount: parsed.filter((l) => l.level === "WARN" || l.level === "WARNING").length,
        patterns,
    };
}
